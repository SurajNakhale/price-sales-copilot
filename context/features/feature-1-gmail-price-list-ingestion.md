...
| Path | Purpose |
|---|---|
| `app/page.tsx` | Server component: connection status, files already saved, renders `IngestPanel` |
| `app/_components/IngestPanel.tsx` | Client component: connect/disconnect, scan, candidate table with checkboxes, download, result list. Uses shadcn `Card`, `Button`, `Table`, `Checkbox`, `Badge`, `Alert` |
...
7. `app/page.tsx` and `IngestPanel.tsx`.
